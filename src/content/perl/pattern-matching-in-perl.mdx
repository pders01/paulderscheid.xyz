...
The problem was that `~~` tried to be smart about what kind of comparison to do based on _both_ operands. String vs number vs array vs hash vs regex vs object — the interaction matrix was enormous and impossible to hold in your head. From `perlop`:
...
The match arm simultaneously tests the shape _and_ binds variables from within it. Elixir does the same:
